...
class Cox(Cox):
    ...
    def embed_specific(self, image, image_file, watermark=None):
        ...
        for i in range(len(nbits)):
            f_dct[sorted_unraveled[i][1]] = math.pow(math.e, (self.alpha * nbits[i]))*f_dct[sorted_unraveled[i][1]]
    ...
    def extract_specific(self, image, watermark):
        ...
        for entry in w:
            ...
            xi.append( math.log((f_dct[tuple(entry[self.INDEX_KEY])]/entry[self.ORIGINAL_VALUE_KEY])) /\
                       (self.alpha))
